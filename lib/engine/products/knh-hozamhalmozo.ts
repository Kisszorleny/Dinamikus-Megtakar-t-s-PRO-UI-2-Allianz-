...
import {
  ...
  buildKnhHozamhalmozoInvestedShareByYear,
  estimateKnhHozamhalmozoDurationYears,
  getKnhHozamhalmozoVariantConfig,
  toKnhHozamhalmozoProductVariantId,
} from "./knh-hozamhalmozo-config"
import type { ProductDefinition } from "./types"

export const knhHozamhalmozo: ProductDefinition = {
  id: "knh-hozamhalmozo",
  ...
  calculate: (inputs: InputsDaily): ResultsDaily => {
    ...
    const durationYears = estimateKnhHozamhalmozoDurationYears(inputs)
    const variantConfig = getKnhHozamhalmozoVariantConfig()

    const initialCostByYearDefault = buildKnhHozamhalmozoInitialCostByYear(inputs, durationYears)
    const investedShareByYearDefault = buildKnhHozamhalmozoInvestedShareByYear(durationYears)
    const assetCostPercentByYearDefault = buildKnhHozamhalmozoAssetCostPercentByYear(durationYears)
    const initialAdminPlusCostByYearDefault = buildKnhHozamhalmozoInitialAdminPlusCostByYear(durationYears)
    const giftBonusByYearDefault = buildKnhHozamhalmozoGiftBonusAmountByYear(inputs, durationYears)

    return calculateResultsDaily({
      ...inputs,
      currency: "HUF",
      productVariant: toKnhHozamhalmozoProductVariantId(),
      durationUnit: "year",
      ...
      adminFeeMonthlyAmount: shouldUseProductDefaults
        ? variantConfig.regularAdminMonthlyAmount
