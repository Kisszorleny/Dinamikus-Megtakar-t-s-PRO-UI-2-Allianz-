...
import {
  ...
  buildKnhNyugdijbiztositas4InvestedShareByYear,
  estimateKnhNyugdijbiztositas4DurationYears,
  getKnhNyugdijbiztositas4VariantConfig,
  toKnhNyugdijbiztositas4ProductVariantId,
} from "./knh-nyugdijbiztositas4-config"
import type { ProductDefinition } from "./types"

export const knhNyugdijbiztositas4: ProductDefinition = {
  id: "knh-nyugdijbiztositas4",
  ...
  calculate: (inputs: InputsDaily): ResultsDaily => {
    ...
    const durationYears = estimateKnhNyugdijbiztositas4DurationYears(inputs)
    const variantConfig = getKnhNyugdijbiztositas4VariantConfig()

    const initialCostByYearDefault = buildKnhNyugdijbiztositas4InitialCostByYear(inputs, durationYears)
    const investedShareByYearDefault = buildKnhNyugdijbiztositas4InvestedShareByYear(durationYears)
    const assetCostPercentByYearDefault = buildKnhNyugdijbiztositas4AssetCostPercentByYear(durationYears)
    const initialAdminPlusCostByYearDefault = buildKnhNyugdijbiztositas4InitialAdminPlusCostByYear(durationYears)
    const giftBonusByYearDefault = buildKnhNyugdijbiztositas4GiftBonusAmountByYear(inputs, durationYears)

    return calculateResultsDaily({
      ...inputs,
      currency: "HUF",
      productVariant: toKnhNyugdijbiztositas4ProductVariantId(),
      durationUnit: "year",
      ...
      extraordinaryAdminFeePercentOfPayment: shouldUseProductDefaults
        ? variantConfig.extraordinaryAcquisitionPercent
      ...
      partialSurrenderFeeAmount: shouldUseProductDefaults
        ? variantConfig.surrenderFeeAmount
      ...
      bonusAmountByYear: shouldUseProductDefaults
        ? { ...giftBonusByYearDefault, ...(inputs.bonusAmountByYear ?? {}) }
      ...
      plusCostByYear: shouldUseProductDefaults
        ? { ...initialAdminPlusCostByYearDefault, ...(inputs.plusCostByYear ?? {}) }
