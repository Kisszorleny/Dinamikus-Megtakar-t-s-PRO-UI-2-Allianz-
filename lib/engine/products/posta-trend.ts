...
import {
  ...
  buildPostaTrendInvestedShareByYear,
  buildPostaTrendRedemptionFeeByYear,
  estimatePostaTrendDurationYears,
  getPostaTrendVariantConfig,
  toPostaTrendProductVariantId,
} from "./posta-trend-config"
import type { ProductDefinition } from "./types"

export const postaTrend: ProductDefinition = {
  id: "posta-trend",
  label: "Posta Trend",
  calculate: (inputs: InputsDaily): ResultsDaily => {
    const shouldUseProductDefaults = inputs.disableProductDefaults !== true
    const durationYears = estimatePostaTrendDurationYears(inputs)
    const variantConfig = getPostaTrendVariantConfig()

    const initialCostByYearDefault = buildPostaTrendInitialCostByYear(durationYears)
    const investedShareByYearDefault = buildPostaTrendInvestedShareByYear(durationYears)
    const assetCostPercentByYearDefault = buildPostaTrendAssetCostPercentByYear(durationYears)
    const redemptionFeeByYearDefault = buildPostaTrendRedemptionFeeByYear(durationYears)

    return calculateResultsDaily({
      ...inputs,
      currency: "HUF",
      productVariant: toPostaTrendProductVariantId(),
      durationUnit: "year",
